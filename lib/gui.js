...
function buildMap () {
    ...
    // event hooks
    for (territory in gui.map) {
        (function (territory, path) {
            // jQuery listeners in here
            $(path.node).click(
                function () {
                    alert("You clicked: " + game.t[territory].name)
                }
            );
        })(territory, gui.map[territory]);
    }
    
    console.log("SVG init complete!");
